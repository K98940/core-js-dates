...
function getNextFriday(date) {
  ...
  const countDaysToFriday = daysMap[day];
  date.setDate(date.getDate() + countDaysToFriday);
  return date;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const different = Math.abs(endDate - startDate);
  return Math.trunc(different / MS_IN_DAY) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  return checkingDate >= periodStart && checkingDate <= periodEnd;
}
...
function getWeekNumberByDate(date) {
  ...
  return Math.ceil((differentDays + shift) / 7);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (pointer <= endDate) {
    for (let i = 0; i < countWorkDays; i += 1) {
      ...
      schedule.push(convertDate(pointer));
...
function isLeapYear(date) {
  const BUT_NOT_100 = 100;
  const year = date.getFullYear();
  if (year === BUT_NOT_100) return false;
  return year % 4 === 0;
}
